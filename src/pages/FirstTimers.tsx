...
export default function FirstTimers() {
  ...
  const fetchFirstTimers = async () => {
    const { data, error } = await supabase.from("members").select("*").eq("status", "First Timer").order("created_at", { ascending: false });
    if (error) toast.error("Failed to load first timers");
    else setFirstTimers(data || []);
  };

  useEffect(() => { fetchFirstTimers(); }, []);
  ...
  const handleRegister = async (e: React.FormEvent) => {
    ...
    try {
      ...
      fetchFirstTimers();
      setFormData({ full_name: "", phone_number: "", email: "", gender: "", age_range: "", address: "", location: "", date_of_first_visit: new Date().toISOString().split("T")[0], service_attended: "Sunday Service" });
    } catch (err: any) {
  ...
  const filteredFirstTimers = firstTimers.filter(ft => 
    ft.full_name?.toLowerCase().includes(searchTerm.toLowerCase()) || ft.phone_number?.includes(searchTerm)
  );
    ...
    <div className="space-y-6">
      <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
        <div>
          ...
          <p className="text-muted-foreground mt-1">Register and track first-time visitors</p>
        </div>
        <div className="flex gap-3 w-full sm:w-auto">
          <Dialog open={isRegisterOpen} onOpenChange={setIsRegisterOpen}>
            <DialogTrigger asChild>
              <Button className="gap-2"><UserPlus className="h-4 w-4" />Register First Timer</Button>
            </DialogTrigger>
            <DialogContent className="max-w-2xl max-h-[90vh] overflow-y-auto bg-card border-[rgba(255,255,255,0.07)]">
              <DialogHeader>
                <DialogTitle className="font-display text-xl">Register New First Timer</DialogTitle>
              </DialogHeader>
              <form onSubmit={handleRegister} className="grid grid-cols-1 md:grid-cols-2 gap-4 mt-4">
                <div className="space-y-2">
                  <Label>Full Name *</Label>
                  <Input required value={formData.full_name} onChange={e => setFormData({...formData, full_name: e.target.value})} className="h-11" />
                </div>
                <div className="space-y-2">
                  <Label>Phone Number</Label>
                  <Input value={formData.phone_number} onChange={e => setFormData({...formData, phone_number: e.target.value})} className="h-11" />
                </div>
                <div className="space-y-2">
                  <Label>Email</Label>
                ...
                <div className="space-y-2">
                  ...
                  <Select value={formData.age_range} onValueChange={v => setFormData({...formData, age_range: v})}>
                    ...
                    <SelectContent>
                      <SelectItem value="Under 18">Under 18</SelectItem><SelectItem value="18-24">18-24</SelectItem>
                      <SelectItem value="25-34">25-34</SelectItem><SelectItem value="35-44">35-44</SelectItem>
                      <SelectItem value="45-54">45-54</SelectItem><SelectItem value="55+">55+</SelectItem>
                    </SelectContent>
                  </Select>
                </div>
                <div className="space-y-2">
                  <Label>Date of First Visit *</Label>
                  <Input type="date" required value={formData.date_of_first_visit} onChange={e => setFormData({...formData, date_of_first_visit: e.target.value})} className="h-11" />
                </div>
                <div className="space-y-2">
                ...
                <div className="space-y-2">
                  ...
                  <Input value={formData.location} onChange={e => setFormData({...formData, location: e.target.value})} className="h-11" />
                </div>
                ...
                <div className="md:col-span-2 flex justify-end gap-2 mt-4">
                  ...
                  <Button type="submit">Save First Timer</Button>
                </div>
              </form>
            </DialogContent>
          </Dialog>
              ...
              <table className="w-full">
                ...
                <tbody className="text-sm">
                  {filteredFirstTimers.map((ft) => (
                    <tr key={ft.id} className="border-b border-[rgba(255,255,255,0.04)] last:border-0 hover:bg-secondary/50 transition-colors">
                      <td className="py-4 font-medium text-foreground">{ft.full_name}</td>
                      ...
                      <td className="py-4">
                        <span className="badge-primary px-2 py-1 rounded-full text-xs font-medium">{ft.service_attended}</span>
                      </td>
                      <td className="py-4 text-muted-foreground">{new Date(ft.date_of_first_visit).toLocaleDateString()}</td>
                    </tr>
                  ))}
                </tbody>
              </table>
